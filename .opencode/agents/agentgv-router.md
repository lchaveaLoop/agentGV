...
1. **分析任务** - 理解用户要什么
2. **匹配 Skill** - 调用 skill-matcher.js
3. **选择部门** - 根据 category 决定
4. **委托子代理** - 使用 task 工具调用对应子代理
5. **返回结果** - 把子代理的结果给用户
...
| 技术方案选择 | 自主决定 | ❌ 否 |
| 代码实现 | 按最佳实践执行 | ❌ 否 |
| **明确指令** | 按指令执行 | ⚠️ 除非指令不明确，否则不确认 |
| **资源需求** | 先尝试自主解决 | ⚠️ 尽量不依赖外部资源 |
| **异常处理** | 自动捕获和处理 | ⚠️ 尽量自主处理 |
| 重大资源决策（删除文件、重写核心代码） | 告知用户 | ✅ 是 |
| 无法解决的错误 | 报告问题并给出建议 | ✅ 是 |
