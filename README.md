...
**当前架构**: 4 个精简高效部门（Intelligence/Communications/Administration 能力已分别合并到 Planning/Operations/Router）
...
### Router (路由器) - 主 Agent
- **职责**: 智能路由，Skill 匹配，项目协调（吸收 Administration 能力）
- **模型**: bailian-coding-plan/qwen3.5-plus
- **模式**: primary
- **特性**: 自主闭环执行，多部门协调，模型自动同步

### Planning (规划局) - 吸收 Intelligence
- **职责**: 架构设计、技术方案、系统规划、技术选型、市场调研、竞品分析
- **模型**: bailian-coding-plan/qwen3.5-plus (复杂任务升级到 qwen3-max)
- **模式**: subagent
- **能力**: 原 Intelligence 部门职责已合并

### Operations (执行部) - 吸收 Communications
- **职责**: 功能开发、编码实现、系统集成、技术文档、用户手册
- **模型**: bailian-coding-plan/qwen3.5-plus (编码优选 qwen3-coder-plus)
- **模式**: subagent
- **能力**: 原 Communications 部门职责已合并
...
- 部门：Operations → Quality
- 模型：qwen3.5-plus

执行流程:
1. @agentgv-operations - 功能开发 + 文档编写
...
| 原部门 | 现归属 | 说明 |
|--------|--------|------|
| Intelligence | Planning | 调研分析能力已合并到规划局 |
| Communications | Operations | 文档编写能力已合并到执行部 |
| Administration | Router | 协调管理能力已合并到路由器 |
